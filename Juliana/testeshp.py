	...
	for y in xrange(yInicial, ysize, y_block_size):
		...
		for x in xrange(xInicial, xsize, x_block_size):
			...
			for i in xrange(len(unique_elements)):
				...
				if(unique_elements[i]==27.0):
					print(x,y,cols,rows,xoff, yoff, xcount, ycount,counts_elements[i])
					guarda = 0
					for t in xrange(len(dataraster)):
						for v in xrange(len(dataraster[t])):
							if(dataraster[t][v]==27):
								guarda = (t,v)
								break
					print(guarda)
					for t in xrange(guarda[0],guarda[0]+5):
						for v in xrange(guarda[1]-5,guarda[1]+5):
							print(dataraster[t][v],datamask[t][v])
					exit(1)
